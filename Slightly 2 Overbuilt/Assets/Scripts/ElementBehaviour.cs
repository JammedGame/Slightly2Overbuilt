...
public class ElementBehaviour : MonoBehaviour
{
	...
	private void CreateFragmentObjects() 
	{
		for(int i = 0; i < this._Data.Fragments.Count; i++)
		{
            ...
            FragmentObject.transform.localScale = new Vector3(Element.Size*50, Element.Size*50, Element.Size*50);
			if (this._Data.Location.x < 0) FragmentObject.transform.position = new Vector3(0, - 1.2f * Element.Size*50, 0);
			else
			{
