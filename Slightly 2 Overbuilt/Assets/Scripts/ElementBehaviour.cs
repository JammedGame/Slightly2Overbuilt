...
public class ElementBehaviour : MonoBehaviour
{
	...
	private void CreateFragmentObjects() 
	{
		for(int i = 0; i < this._Data.Fragments.Count; i++)
		{
			...
			cube.transform.position = new Vector3(((this._Data.Location.x - 2) * Element.Size) + this._Data.Fragments[i].Offset.x, 0.5f, - ((this._Data.Location.y - 2) * Element.Size) - this._Data.Fragments[i].Offset.z);
